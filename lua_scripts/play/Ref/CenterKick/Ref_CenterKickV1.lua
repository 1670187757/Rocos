...
local p1 = CGeoPoint:new_local(-2000,0)
...
["v"] = {
	switch = function()
		...
		-- end
		-- return "getball"
		-- debugEngine:gui_debug_arc(player.pos("Assister"),500,360,360,1)
		-- debugEngine:gui_debug_arc(CGeoPoint:new_local(0,0),500,360,360,1)
	end,
...
["pass"] = {
	...
	Kicker = task.Shootdot(pos_self("Assister"),false,0.12,8,1), -- p之后改成最佳射门点
	match = "{AK}"
},
...
["shoot1"] = {
	...
	Assister = task.Shootdot(GoalPos(),false,0.2,10,1), -- p之后改成最佳射门点
	Kicker = task.goCmuRush(p1,_,_,flag.dodge_ball + flag.allow_dss),
	match = "{AK}"
},
